...
const StockIn: React.FC = () => {
  ...
  const fetchHistory = async () => {
    try {
      ...
      const mapped: StockItem[] = res.data
        .filter((s) => s.type === "STOCK_IN")
        .map((s) => ({
          id: s.id,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      /* 2️⃣ UPDATE ITEM STOCK */
      if (savedId) {
        await axios.put("http://localhost:8080/api/staff/stock/update", null, {
          params: {
  ...
  const totalIn = history.reduce((sum, s) => sum + safeNumber(s.quantity), 0);

  return (
    <div className="p-3 sm:p-4 md:p-6 space-y-5">
      <Breadcrumb page="Stock In" parent="Stock" />
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p style={{ color: tokens.muted }}>Record incoming inventory</p>
        </div>
      ...
      >
        ...
        >
          <h3 className="font-semibold text-sm">Stock In History</h3>
        </div>
