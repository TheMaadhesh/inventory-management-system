...
import {
    ...
    CheckCircleIcon, PackageIcon, SunIcon, MoonIcon, ShieldCheckIcon,
} from "lucide-react";
        ...
        <motion.div key="success"
            ...
            <div className="flex flex-col items-center gap-5 text-center py-3">
                ...
                <button onClick={onContinue}
                    ...
                    style={{ background: t.btnBg, border: `1px solid ${t.btnBdr}`, boxShadow: t.btnShadow }}>
                    Go to Admin Dashboard <ArrowRightIcon className="size-4" />
                </button>
            </div>
        </motion.div>
    );
...
export default function AdminLoginPage({ onLoginSuccess }) {
    ...
    /* Step 1 → verify credentials → send OTP → go to step 2 */
    const handleSendOtp = async () => {
        ...
        try {
            ...
            if (payload.role !== "ADMIN") {
                setLoginError("This account does not have Admin access. Please use Staff Login.");
                return;
            }

            // Store token temporarily (OTP not verified yet – will be confirmed after OTP)
    ...
    /* OTP confirmed → promote pending token to real token → redirect */
        ...
        <div className="min-h-screen w-full flex flex-col overflow-x-hidden" style={{ background: t.pageBg }}>
            ...
            <div className="fixed inset-0 pointer-events-none overflow-hidden" style={{ zIndex: 0 }}>
                {isDark ? (<>
                    ...
                    <div className="absolute -bottom-36 -right-16 w-[380px] h-[380px] sm:w-[680px] sm:h-[680px] rounded-full"
                        style={{ background: "radial-gradient(circle,rgba(99,102,241,0.14) 0%,transparent 65%)" }} />
                    <div className="absolute top-1/2 right-1/4 w-[200px] h-[200px] sm:w-[380px] sm:h-[380px] rounded-full"
                    ...
                    <div className="absolute top-1/3 left-1/2 -translate-x-1/2 w-[400px] h-[400px] rounded-full"
                        style={{ background: "radial-gradient(circle,var(--orb3) 0%,transparent 70%)" }} />
                </>)}
            </div>
                ...
                <motion.div className="w-full max-w-[420px] sm:max-w-md"
                    ...
                    <div className="flex items-center gap-2.5 mb-8 justify-center">
                        <div className="w-9 h-9 rounded-xl flex items-center justify-center shadow-lg"
                            ...
                            <ShieldCheckIcon className="size-5 text-white" strokeWidth={1.8} />
                        </div>
                        <div className="leading-tight">
                            <p className="text-sm font-extrabold tracking-wide" style={{ color: t.txtMain }}>IIM — Admin</p>
                        ...
                        <div className="flex items-center justify-center gap-3 mb-7">
                            {[1, 2].map(s => (
                                <div key={s} className="flex items-center gap-3">
                                    ...
                                    {s === 1 && <div className="w-14 h-px transition-all duration-500" style={{ background: t.stepLine }} />}
                                </div>
                            ))}
                        </div>
                    )}
                    ...
                    <div className="rounded-2xl p-6 sm:p-9"
                        style={{
                            ...
                            backdropFilter: "blur(28px)", WebkitBackdropFilter: "blur(28px)", boxShadow: t.cardSdw,
                        }}>
                        <AnimatePresence mode="wait">
                            ...
                            {step === 1 && (
                                <motion.div key="login"
                                    initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -24 }}
                                    transition={{ duration: 0.2 }}>
                                    <h1 className="text-xl sm:text-2xl font-bold mb-1" style={{ color: t.txtMain }}>Admin Login</h1>
                                    ...
                                    <div className="flex flex-col gap-3.5">
                                        <Field name="email" label="Email Address" type="email"
                                            placeholder="admin@company.com" value={email} error={errors.email}
                                    ...
                                    <p className="text-xs text-center mt-5" style={{ color: t.txtMuted }}>
                                        ...
                                        <button onClick={() => navigate("/signup")}
                                            ...
                                            style={{ color: t.accentCl }}>Sign up</button>
                                    </p>
                                    <p className="text-xs text-center mt-2" style={{ color: t.txtMuted }}>
                                        Staff member?{" "}
                                        <button onClick={() => navigate("/staff/login")}
                                            className="font-medium hover:underline underline-offset-2"
                                            style={{ color: t.accentCl }}>Staff Login →</button>
                                    </p>
                                </motion.div>
                            )}

                            {step === 2 && (
                                <OtpStep email={email} onLogin={handleLoginSuccess} onBack={() => setStep(1)} t={t} />
                            )}
                            {step === 3 && (
                                <SuccessStep t={t} onContinue={goDashboard} />
                            )}
